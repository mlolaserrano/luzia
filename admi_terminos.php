<?php
// Funciones auxiliares para leer/escribir archivos de texto
...
$currentTitle = getTermsData('title') ?: '¡Estamos en construcción!';
$currentDescription = getTermsData('description') ?: 'Lamentamos los inconvenientes. Nuestros términos y condiciones están siendo actualizados.';
$currentImage = getTermsData('image_path');
$message = "";

// Manejar mensaje desde GET (por ejemplo, desde redirección de carga)
if (isset($_GET['msg'])) {
    $message = urldecode($_GET['msg']);
}

// Manejar guardado de texto (desde modalTexto)
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['save_text'])) {
    $newTitle = $_POST['terms_title'];
    $newDescription = $_POST['terms_description'];
    setTermsData('title', $newTitle);
    setTermsData('description', $newDescription);
    $currentTitle = $newTitle;
    $currentDescription = $newDescription;
    $message = "¡Texto actualizado con éxito!";
}
?>
...
<head>
  ...
  <title>Administrador | Términos y Condiciones</title>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form method="POST" enctype="multipart/form-data" action="upload_terminos.php">
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body text-center">
        ...
        <p class="mt-3">¿Estás seguro de que querés eliminar esta imagen?</p>
        <div class="alert alert-warning">Esta acción no se puede deshacer.</div>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-luzia" data-bs-dismiss="modal">Cancelar</button>
        <form method="POST" action="upload_terminos.php" style="display: inline;">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form method="POST">
          ...
          <input type="text" class="form-control mb-3" name="terms_title" value="<?php echo htmlspecialchars($currentTitle); ?>" required>
          <label class="form-label">Descripción</label>
          <textarea class="form-control" name="terms_description" rows="4" required><?php echo htmlspecialchars($currentDescription); ?></textarea>
      </div>
      <div class="modal-footer">
