...
// Funciones auxiliares (mismas que en admi_politicas.php)
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['upload_image'])) {
    if (isset($_FILES['image'])) {
        ...
        if ($error !== UPLOAD_ERR_OK) {
            ...
            // Proceed with validation if no upload error
            ...
            if (!is_writable($targetDir)) {
                ...
                if ($imageInfo === false || $imageInfo[0] != 600 || $imageInfo[1] != 400) {
                    ...
                    if (move_uploaded_file($file['tmp_name'], $targetFile)) {
                        setPolicyData('image_path', $targetFile);
...
// Manejar eliminación de imagene
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['delete_image'])) {
    if (!empty($currentImage) && file_exists($currentImage)) {
        unlink($currentImage);
    }
    setPolicyData('image_path', '');
    $message = "¡Imagen eliminada exitosamente!";
}

// Redirigir a la página principal con mensaje
header("Location: admi_politicas.php?msg=" . urlencode($message));
exit;
?>